...
df['date'] = pd.to_datetime(df['caldt'], format='%d/%m/%Y')
...
monthly_market_returns = monthly_market_returns.rename('mkt_ret')
...
combined_monthly_df = pd.merge(monthly_market_returns, ff_df['rf'], how='inner', on='date')
monthly_excess_ret = combined_monthly_df['mkt_ret'] - combined_monthly_df['rf']

# The factor to be compounded
...
def calculate_forward_returns(horizon):
    ...
    for date in monthly_returns_plus_1.index:
        ...
        # Can no longer be calculated at the end of the sample
